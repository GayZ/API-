h2. object Address

|name|string|mandatory|Форматированное наименование|
|components|array [[object AddressComponent|Адресные-объекты#object-addresscomponent]]|optional|Компоненты|
|types|object AddressTypes|optional|Идентификатор типа|
|location|object AddressLocation|optional|Координаты|

h2. object AddressComponent

|level|number AddressLevel|mandatory|Уровень|
|name|string|mandatory|Наименование|

h2. object AddressTypes

|pointType|number PointType|optional|Идентификатор типа точки|
|pointTypeName|string|optional|Наименование типа точки|
|aliasType|number AliasType|optional|Идентификатор типа алиаса|
|aliasTypeName|string|optional|Наименование типа алиаса|

h2. object AddressLocation

|lon|number|mandatory|Долгота|
|lat|number|mandatory|Широта|

h2. number AddressLevel

|0|Страна|
|1|Административный уровень 1|
|2|Административный уровень 2|
|3|Административный уровень 3|
|4|Административный уровень 4|
|5|Административный уровень 5|
|6|Административный уровень 6|
|7|Улица|
|8|Дом|
|9|Алиас|

h2. number PointType

h2. number AliasType
