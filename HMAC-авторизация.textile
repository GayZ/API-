...
public class NaiveHmacSigner {
    ...
    public NaiveHmacSigner(String username, Mac mac) {
        this.mac = mac;
        this.username = username;
    }


    public synchronized Map<String, String> newSignatureHeaders(final String method, final String uri) {

        final String dateHeaderValue = OffsetDateTime.now().format(DateTimeFormatter.RFC_1123_DATE_TIME);
        final String nonce = "" + Math.abs(this.rnd.nextInt());
        final String signatureData = method + uri + dateHeaderValue + nonce;

        this.mac.reset();
        final byte[] signatureBytes = this.mac.doFinal(signatureData.getBytes(UTF8CHARSET));
        final String signatureBase64 = Base64.getEncoder().encodeToString(signatureBytes);
        final String authHeaderValue = String.format("hmac %s:%s:%s", this.username, nonce, signatureBase64);
    ...
    public static NaiveHmacSigner newSigner(final String username, final byte[] secret)
        ...
        mac.init(spec);
        return new NaiveHmacSigner(username, mac);
    }
}
```