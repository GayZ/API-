...
** *digest* - подпись сообщения, вычисляемая как @base64encode(hmac("sha256", {secret}, {method}{path}{date}{nonce}))@, где
*** *secret* – секретный ключ, расшаренный между клиентом и сервисом.
