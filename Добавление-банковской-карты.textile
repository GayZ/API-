...
В ответ на этт запрос полоьзователь получит ссылку на страницу банка для ввода данных карты. После ввода данных карты на этой странице, банк перенаправит пользователя по некоторому callback URL на строне приложения hive. Нужно обязательно дождаться ответа сервера. В ответ сервер вернет JSON документ @object CardAdditionCallback@
